...
| `clinic_command.html` | A **"Call these patients today"** panel appears above the sample cases, filled from `GET /api/clinic/call-list`: most urgent first, the patient's reply, follow-up day and language. **Mark as called** asks for confirmation, then records successful contact only for items present at the last queue snapshot (newer replies/readings remain open); it shows in the patient's "who viewed my record" log. **Open visit** goes to that patient's visit screen. The acuity counts at the top become real. It refreshes every 15 seconds. |
