...
The hosted verification workflow rehearses a PostgreSQL 16 custom-format backup and restore using disposable CI databases. Run [36104133960](https://github.com/tanhongsheng050204-code/KHABAR-HEALTHCARE/actions/runs/36104133960) passed: it checked a marker row and restored Flyway migration history. This verifies the database toolchain on synthetic CI data; it does not rehearse a production backup, restore writes that occurred after a backup, a failed/forward migration recovery, or Vercel deployment rollback.
