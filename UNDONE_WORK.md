...
- Next.js web app: lint, TypeScript, and the production build pass. The build needed to run outside the restricted sandbox because its worker could not spawn there (`EPERM`). Hosted verification for branch head `954b5c9` passed all jobs on 25 Sep (run [36105880641](https://github.com/tanhongsheng050204-code/KHABAR-HEALTHCARE/actions/runs/36105880641)); this is CI evidence only. A fresh read-only route check on 25 Sep returned web 200, API `/api/health` timed out, and `/agents/health` returned 404. Deployment and successful live API/agent health checks remain open.
