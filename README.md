...
- **Verification (25 Sep):** local checks passed: 226 API tests (one optional PostgreSQL test skipped locally), 205 agent tests, and frontend lint, TypeScript and production build. Hosted CI passed all jobs for branch head `d66e873` ([run 36109309203](https://github.com/tanhongsheng050204-code/KHABAR-HEALTHCARE/actions/runs/36109309203)), including PostgreSQL migration/schema and point-in-time backup/restore checks. CI does not deploy the branch. Fresh read-only GETs returned web 200 and API `/api/health` 200; both `/agents/health` and `/agents/agents/health` returned 404. Live deployment verification remains open.
