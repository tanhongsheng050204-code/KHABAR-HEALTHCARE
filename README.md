...
- **Verification (25 Sep):** local checks passed: 226 API tests (one optional PostgreSQL test skipped locally), 205 agent tests, and frontend lint, TypeScript and production build. Hosted CI passed all jobs for branch head `1bbdd10` ([run 36110130766](https://github.com/tanhongsheng050204-code/KHABAR-HEALTHCARE/actions/runs/36110130766)), including API tests, PostgreSQL migration/schema checks and point-in-time backup/restore. This run also verifies the corrected graph privacy assertion. CI does not deploy the branch. Fresh read-only GETs returned web 200 and API `/api/health` 200; both `/agents/health` and `/agents/agents/health` returned 404. Live deployment verification remains open.
...
- **Local rehearsal (25 Sep):** doctor and patient demo flow, duplicate-safety gate, final summary, follow-up response, caregiver consent denial after revocation, and agent-outage fallback exercised with fictional data. It did not verify the current public deployment or real sign-in; see [the local run record](docs/DEMO_RUN_2026-09-25_LOCAL.md).
