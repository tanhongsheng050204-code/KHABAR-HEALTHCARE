...
- **Verification (25 Sep):** local checks passed: 226 API tests (one optional PostgreSQL test skipped locally), 205 agent tests, and frontend lint, TypeScript and production build. Hosted CI also passed all jobs for branch head `954b5c9` ([run 36105880641](https://github.com/tanhongsheng050204-code/KHABAR-HEALTHCARE/actions/runs/36105880641)), including PostgreSQL migration/schema and disposable backup/restore checks. These checks do not deploy the branch. Fresh read-only checks returned web 200 and API `/api/health` 200 (one earlier request timed out); `/agents/health` still returned 404. Live deployment verification remains open.
