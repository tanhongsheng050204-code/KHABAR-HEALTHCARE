...
|---|---|---|
| Sign-in and access | Supabase token checks; doctor, patient and caregiver access rules; revoked caregiver consent blocks access at once; the sign-in screen has email and password for clinic staff and an email code for patients and caregivers, plus the demo sign-in | Connecting the screen to a configured Supabase project and testing every role on the live site |
...
| Before the visit | Self-booking from a rolling calendar (the database refuses double bookings); intake chat that confirms what the clinic already knows; the pre-visit page (intake, last visit, the medicine list checked for duplicates across clinics, clashes, herbs and allergies, replies since); a "what I take" list kept by the patient, caregiver or clinic | Packet photos: built behind explicit consent (the image is read by Gemini and never stored; the patient reviews each item before it is added), tested end to end with a stand-in model, not yet tried with real Gemini on fictional packets |
